...
function WorkspaceApp() {
  ...
  const [pendingTeamDeletion, setPendingTeamDeletion] = useState<{ ids: number[]; name: string } | null>(null);
  ...
  const handleSend = (value = input, attachmentContext?: Attachment) => {
    ...
    replyMutation.mutate({ channel: selectedId, teamName: selectedChat.title, businessName: businessName.trim() || "Bisnismu", persona: persona.trim() || undefined, agentName: selectedAgent.name, agentRole: selectedAgent.role, skills: selectedAgent.skills, memory: selectedAgent.memory, dataAccess: selectedAgent.dataAccess, pipeline: selectedAgent.pipeline, automation: selectedAgent.automation, history: currentMessages }, {
      onSuccess: (response) => {
        ...
        const actionResult = response.toolResults?.[0] as { toolName?: string; automation?: AutomationRecord; caption?: string; imageUrl?: string; platform?: "instagram" | "twitter" | "general"; confirmationRequired?: boolean; divisionId?: number; divisionIds?: number[]; divisionName?: string } | undefined;
        if (actionResult?.toolName === "delete_division" && actionResult.confirmationRequired && actionResult.divisionName) setPendingTeamDeletion({ ids: actionResult.divisionIds?.length ? actionResult.divisionIds : actionResult.divisionId ? [actionResult.divisionId] : [], name: actionResult.divisionName });
        ...
        const actionLabel = actionResult?.toolName === "create_division" ? "Divisi dibuat dan disimpan" : actionResult?.toolName === "update_division" ? "Tim diperbarui" : actionResult?.toolName === "delete_division" ? "Tim dihapus" : actionResult?.toolName === "recommend_team_structure" ? "Struktur tim dianalisis" : actionResult?.toolName === "create_automation" ? "Automasi dibuat dan diaktifkan" : actionResult?.toolName === "list_divisions" ? "Daftar divisi diperbarui" : actionResult?.toolName === "list_automations" ? "Daftar automasi diperbarui" : actionResult?.toolName === "advance_pipeline" ? "Progress pipeline diperbarui" : actionResult?.toolName === "record_finance_transaction" ? "Transaksi diproses ke Jurnal Finance" : actionResult?.toolName === "generate_image" ? "Visual dibuat dan dilampirkan" : actionResult?.toolName === "generate_video" ? "Video dibuat dan dilampirkan" : actionResult?.toolName === "generate_voice_note" ? "Voice note dibuat dan dilampirkan" : actionResult?.toolName === "generate_content_package" ? "Paket konten siap direview" : undefined;

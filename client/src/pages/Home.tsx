    ...
    <button onClick={onClick} className={`chat-list-item w-full text-left ${selected ? "chat-list-item-selected" : ""}`}>
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center justify-between gap-2">
          ...
          <span className={`shrink-0 text-[10px] ${selected ? "text-[#337c6c]" : "text-[#9aa19e]"}`}>{chat.time}</span>
        </span>
        <span className="mt-1 flex items-center justify-between gap-2">
          <span className={`truncate text-[12px] ${chat.typing ? "font-medium text-[#258168]" : "text-[#7a8581]"}`}>
            {chat.typing ? "sedang mengetik…" : chat.preview}
          </span>
          {chat.unread ? <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-[#2d977b] px-1.5 text-[10px] font-bold text-white">{chat.unread}</span> : null}
        </span>
      </span>
    </button>
  );
  ...
  const content = (
    <span className={`flex items-center gap-3 ${compact ? "min-w-0" : ""}`}>
      ...
      <span className="min-w-0 text-left">
        <span className="block truncate text-[12px] font-semibold">{attachment.name}</span>
        {attachment.size && <span className="mt-0.5 block text-[10px] opacity-65">{attachment.size}</span>}
        {statusLabel && <span data-testid="extraction-status" className="mt-1 block text-[10px] font-semibold text-[#2d856d]">{statusLabel}</span>}
        {attachment.extractionPreview && <span className="mt-1 block max-h-10 overflow-hidden text-[10px] leading-4 text-[#789086]">{attachment.extractionPreview}</span>}
        {compact && attachment.extractedText && <details className="mt-2 text-[10px] text-[#617c70]"><summary className="cursor-pointer font-semibold">Buka konten terbaca</summary><pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap rounded-lg bg-[#f7fbf8] p-2 font-sans leading-4">{attachment.extractedText}</pre></details>}
      </span>
    </span>
  );
...
function ContentPackageCard({ contentPackage }: { contentPackage: NonNullable<ChatMessage["contentPackage"]> }) {
  ...
  return <div className="mt-3 overflow-hidden rounded-xl border border-[#cfe8dc] bg-[#f7fcf9]">
    ...
    <div className="p-3">
      <div className="mb-2 flex items-center justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#39836f]">Paket konten · {contentPackage.platform}</span><span className="text-[10px] text-[#82918b]">Posting manual</span></div>
      <p className="whitespace-pre-wrap text-[12px] leading-5 text-[#3f514a]">{contentPackage.caption}</p>
      <div className="mt-3 flex flex-wrap gap-2">
        <button type="button" onClick={copyCaption} className="rounded-lg bg-[#2d8d73] px-3 py-2 text-[11px] font-bold text-white transition hover:bg-[#24765f]">{copied ? "Caption tersalin" : "Salin Caption"}</button>
        {contentPackage.imageUrl && <a href={contentPackage.imageUrl} download="saku-content-package.png" className="rounded-lg border border-[#b9d9ca] bg-white px-3 py-2 text-[11px] font-bold text-[#39735f] transition hover:bg-[#edf8f1]">Unduh Gambar</a>}
      </div>
    </div>
  </div>;
}
  ...
  return (
    <div className={`message-row ${isOwner ? "justify-end" : "justify-start"}`}>
      ...
      <div className={`message-stack ${isOwner ? "items-end" : "items-start"}`}>
        {!isOwner && <span className="mb-1 px-1 text-[10px] font-semibold text-[#4b8b7a]">{message.senderName} <span className="font-normal text-[#9aa29f]">· {message.senderRole}</span></span>}
        <div className={`message-bubble ${isOwner ? "message-bubble-owner" : "message-bubble-team"}`}>
          {displayText && <p className="whitespace-pre-wrap text-[13px] leading-[1.55]">{displayText}</p>}
          {message.actionLabel && <div className="mt-2 flex items-center gap-1.5 rounded-lg bg-[#eff8f3] px-2 py-1.5 text-[10px] font-semibold text-[#2b8068]"><Check className="h-3 w-3" /> {message.actionLabel}</div>}
          ...
          <div className={`mt-1.5 flex items-center justify-end gap-1.5 text-[10px] ${isOwner ? "text-[#6c9c8e]" : "text-[#9da7a3]"}`}>
            {message.reactions?.map((reaction) => <span key={reaction} className="mr-auto rounded-full bg-white/70 px-1.5 py-0.5 text-[11px]">{reaction}</span>)}
    ...
    <aside className="details-panel">
      ...
      <div className="details-scroll">
        <div className="flex flex-col items-center px-6 pb-5 pt-7 text-center">
          ...
          <p className="mt-1 text-[12px] text-[#83908b]">{chat.subtitle}</p>
        </div>
        ...
        {tab === "about" && (
          <div className="space-y-5 p-5">
            <div>
              ...
              <p className="mt-2 text-[13px] leading-6 text-[#65716e]">{chat.description}</p>
            </div>
            <div className="employee-profile-card"><div className="flex items-center gap-2"><span className="agent-context-status" /><div><p className="text-[12px] font-bold text-[#30443d]">{agent.name} · {agent.role}</p><p className="text-[10px] text-[#7f9289]">AI employee · siap bekerja</p></div></div><div className="mt-3 flex flex-wrap gap-1.5">{agent.skills.map((skill) => <span key={skill} className="rounded-full bg-white px-2 py-1 text-[10px] font-semibold text-[#5c776b]">{skill}</span>)}</div></div><div className="rounded-xl border border-[#dfece5] bg-[#f8fcfa] p-3"><p className="eyebrow">Konfigurasi aktif yang dipakai AI</p>{persistedContext?.division && <p className="mt-2 text-[11px] leading-5 text-[#65716e]"><strong>{persistedContext.division.businessArea}</strong> · {persistedContext.division.description}</p>}{persistedContext?.agent?.personality && <p className="mt-2 text-[11px] leading-5 text-[#65716e]"><strong>Personality:</strong> {persistedContext.agent.personality}</p>}{persistedContext?.standards && <div className="mt-3 space-y-1 text-[10px] leading-4 text-[#71847a]"><p><strong>Tujuan:</strong> {persistedContext.standards.purpose}</p><p><strong>Format:</strong> {persistedContext.standards.outputFormat}</p><p><strong>Guardrail:</strong> {persistedContext.standards.guardrails}</p><p><strong>Checklist:</strong> {persistedContext.standards.checklist}</p></div>}{persistedContext?.pipelines?.[0] && <p className="mt-2 text-[10px] leading-4 text-[#71847a]"><strong>Pipeline:</strong> {persistedContext.pipelines[0].name} · langkah {persistedContext.pipelines[0].currentStep} · {persistedContext.pipelines[0].status}</p>}{persistedContext?.automations?.[0] && <p className="mt-2 text-[10px] leading-4 text-[#71847a]"><strong>Automasi:</strong> {persistedContext.automations[0].name} · {persistedContext.automations[0].trigger} · {persistedContext.automations[0].status}</p>}</div>
            <div className="space-y-4"><div><p className="eyebrow">Memory kerja</p><div className="mt-2 space-y-1.5">{agent.memory.map((memory) => <p key={memory} className="rounded-lg bg-[#f8fbf9] px-3 py-2 text-[11px] leading-5 text-[#6d7d76]">{memory}</p>)}</div></div><div><p className="eyebrow">Database yang bisa dibaca</p><div className="mt-2 flex flex-wrap gap-1.5">{[...agent.dataAccess, ...(workspaceLabel ? [workspaceLabel] : [])].map((item) => <span key={item} className="rounded-full bg-[#f7faf8] px-2 py-1 text-[10px] font-semibold text-[#6d7d76]">{item}</span>)}</div></div><div><p className="eyebrow">Pipeline aktif</p><div className="mt-2 space-y-2">{agent.pipeline.split(" → ").map((step, index) => <div key={step} className="flex items-center gap-2"><span className={`flex h-5 w-5 items-center justify-center rounded-full text-[9px] font-bold ${index + 1 <= (agent.currentStep || 1) ? "bg-[#2d8d72] text-white" : "bg-[#e6f3ec] text-[#4e907a]"}`}>{index + 1}</span><span className="text-[11px] font-semibold text-[#667770]">{step}</span>{index < agent.pipeline.split(" → ").length - 1 && <span className="ml-auto text-[#b1c8bd]">→</span>}</div>)}</div></div><div><p className="eyebrow">Automasi aktif</p><div className="mt-2 flex items-center justify-between rounded-lg border border-[#dceee5] bg-[#f4fbf7] px-3 py-2"><div><p className="text-[11px] font-bold text-[#3d6759]">{agent.automation}</p><p className="mt-0.5 text-[10px] text-[#89a096]">Menunggu pemicu berikutnya</p></div><span className="rounded-full bg-[#dff4e9] px-2 py-1 text-[9px] font-bold text-[#2d856d]">Aktif</span></div></div></div>
            <div>
              <div className="flex items-center justify-between"><p className="eyebrow">Anggota · {chat.members.length}</p><button type="button" onClick={onManageMembers} className="text-[11px] font-semibold text-[#2b8b73]">Kelola</button></div>
              <div className="mt-3 space-y-3">
                {chat.members.map((member) => <div key={member.name} className="flex items-center gap-3"><Avatar initials={member.initials} avatarClass={member.avatarClass} size="sm" online={member.online} /><div className="min-w-0"><p className="text-[12px] font-bold text-[#33413d]">{member.name} <span className="font-normal text-[#a0aaa6]">· {member.role}</span></p><p className="mt-0.5 truncate text-[11px] text-[#8c9894]">{member.bio}</p></div></div>)}
              </div>
            </div>
            <div className="details-setting-row"><span className="flex items-center gap-3"><span className="details-setting-icon"><BellOff className="h-4 w-4" /></span><span><span className="block text-[12px] font-semibold text-[#3b4844]">Bisukan notifikasi</span><span className="block text-[10px] text-[#98a19e]">Hanya untuk grup ini</span></span></span><Toggle checked={muted} onChange={setMuted} label="Bisukan notifikasi" /></div>
        ...
        {tab === "media" && (
          <div className="p-5">
            ...
            <p className="mt-7 text-[11px] leading-5 text-[#99a29f]">Media yang dikirim ke percakapan ini akan tersimpan rapi di sini. Kamu bisa membukanya lagi kapan saja.</p>
          </div>
        )}
        {tab === "docs" && (
          <div className="p-5">
            <div className="flex items-center justify-between"><p className="eyebrow">Dokumen · {allDocs.length}</p><button type="button" onClick={() => setDocFilter((current) => current ? "" : "pdf")} className="text-[11px] font-semibold text-[#2b8b73]">{docFilter ? "Tampilkan semua" : "Filter PDF"}</button></div>
            ...
            <div className="mt-6 rounded-2xl bg-[#f3f8f5] p-4"><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#2b8b73]" /><p className="text-[11px] leading-5 text-[#628278]">Dokumen disimpan di penyimpanan aman dan hanya dibagikan di ruang kerja ini.</p></div></div>
          </div>
        )}
      </div>
      {previewDoc && <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#20322e]/30 p-4" role="dialog" aria-label={`Preview ${previewDoc.name}`} onClick={() => setPreviewDoc(null)}><div className="max-h-[80vh] w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="flex items-center justify-between border-b border-[#e6eee9] px-5 py-4"><div><p className="text-[13px] font-bold text-[#30443d]">Preview dokumen</p><p className="mt-1 text-[11px] text-[#809088]">{previewDoc.name} · {previewDoc.detectedKind || "file"}</p></div><button type="button" aria-label="Tutup preview dokumen" onClick={() => setPreviewDoc(null)} className="rounded-full p-2 text-[#71827a] hover:bg-[#f3f8f5]"><X className="h-4 w-4" /></button></div><div className="max-h-[65vh] overflow-auto p-5"><div className="mb-4 rounded-xl bg-[#f4fbf7] px-3 py-2 text-[11px] font-semibold text-[#2d856d]">{previewDoc.extractionStatus === "complete" ? "Konten terbaca" : "Konten belum dapat diekstrak"}</div><pre data-testid="document-preview" className="whitespace-pre-wrap text-[11px] leading-5 text-[#52665d]">{previewDoc.extractedText || previewDoc.extractionPreview || "Belum ada preview terstruktur untuk dokumen ini."}</pre></div></div></div>}
    </aside>
  );
}
    ...
    <div className="modal-backdrop">
      <div className="onboarding-modal">
        ...
        <div className="px-7 pb-7 pt-6 sm:px-9 sm:pb-9">
          ...
          {step > 0 && <button type="button" aria-label="Kembali ke pertanyaan sebelumnya" disabled={isSaving} onClick={() => setStep((current) => Math.max(0, current - 1))} className="mt-5 inline-flex items-center gap-1.5 rounded-lg px-2 py-1.5 text-[11px] font-bold text-[#5c8173] transition hover:bg-[#f1f7f3] disabled:cursor-not-allowed disabled:opacity-45"><ChevronLeft className="h-3.5 w-3.5" /> Kembali</button>}
...
function MemberManagementPanel() {
  ...
  return <section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e6f0fb] text-[#4f78a5]"><Users className="h-4 w-4" /></span><div><h2>Kelola anggota workspace</h2><p>Catat undangan dan atur akses tim dengan aman</p></div></div><div className="rounded-xl border border-[#dfece5] bg-[#fbfefc] p-3"><p className="text-[11px] leading-5 text-[#74817c]">Setelah diundang, anggota cukup membuat atau masuk ke akun dengan <strong>email yang sama</strong>. SAKU AI akan menampilkan undangan untuk diterima dan mengaktifkan workspace secara otomatis.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><input aria-label="Nama anggota baru" value={name} onChange={(event) => setName(event.target.value)} placeholder="Nama anggota" className="soft-input" /><input aria-label="Email anggota baru" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="email@bisnis.com" type="email" className="soft-input" /></div><div className="mt-2 flex flex-wrap gap-2"><select aria-label="Peran anggota baru" value={role} onChange={(event) => setRole(event.target.value as "member" | "admin")} className="soft-input min-w-32"><option value="member">Member</option><option value="admin">Admin</option></select><button type="button" disabled={inviteMutation.isPending || name.trim().length < 2 || !email.includes("@")} onClick={() => inviteMutation.mutate({ name: name.trim(), email: email.trim(), role })} className="primary-button flex-1 justify-center text-[11px]">{inviteMutation.isPending ? "Menyimpan…" : "Catat undangan"}</button></div></div><div className="mt-4 space-y-2">{isLoading ? <p className="text-[11px] text-[#84918b]">Memuat anggota…</p> : (members as WorkspaceMemberView[]).length ? (members as WorkspaceMemberView[]).map((member) => <div key={member.id} className="flex items-center gap-3 rounded-xl border border-[#e7efeb] bg-white px-3 py-2.5"><Avatar initials={member.name.slice(0, 1).toUpperCase()} avatarClass="bg-[#f2d9c3] text-[#a4612e]" size="sm" /><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-bold text-[#34433e]">{member.name}</p><p className="truncate text-[10px] text-[#8b9892]">{member.email}</p></div><select aria-label={`Peran ${member.name}`} value={member.role} onChange={(event) => updateMutation.mutate({ id: member.id, role: event.target.value as "member" | "admin" })} className="rounded-lg border border-[#dfece5] bg-white px-2 py-1.5 text-[10px] font-semibold text-[#5c7168]"><option value="member">Member</option><option value="admin">Admin</option></select><button type="button" disabled={member.status === "removed"} onClick={() => updateMutation.mutate({ id: member.id, status: "removed" })} className="rounded-lg px-2 py-1.5 text-[10px] font-bold text-[#b86464] hover:bg-[#fff1f1]">Nonaktifkan</button><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${member.status === "active" ? "bg-[#e3f4ec] text-[#2b836b]" : member.status === "removed" ? "bg-[#f8e5e5] text-[#b86464]" : "bg-[#fff0db] text-[#bd762b]"}`}>{member.status === "active" ? "Aktif" : member.status === "removed" ? "Nonaktif" : "Pending"}</span></div>) : <div className="rounded-xl bg-[#f6faf8] p-3 text-[11px] leading-5 text-[#7e8b86]">Belum ada anggota tambahan. Catat undangan pertama di atas.</div>}</div></section>;
}
...
function TeamStandardsPanel({ pipelineTargets }: { pipelineTargets: { channelId: string; title: string }[] }) {
  ...
  return <section className="settings-card" data-testid="team-standards-panel"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e9e5f8] text-[#755fa4]"><ShieldCheck className="h-4 w-4" /></span><div><h2>Standar Tim AI</h2><p>Jaga cara berpikir setiap divisi tetap fokus dan konsisten</p></div></div><div className="rounded-xl border border-[#dfece5] bg-[#f8fcfa] p-3"><label className="settings-field mt-0"><span className="settings-field-label">Pilih tim yang ingin diatur</span><select aria-label="Pilih tim untuk standar" value={selectedChannel} onChange={(event) => setSelectedChannel(event.target.value)} className="soft-input">{targets.map((target) => <option key={target.channelId} value={target.channelId}>{target.title}</option>)}</select></label><div className="mt-3 flex items-start gap-2 rounded-xl bg-[#eef8f2] px-3 py-2.5 text-[11px] leading-5 text-[#55766a]"><ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#2b836b]" /><p>Standar ini dikirim sebagai aturan kerja wajib ke AI tim yang dipilih. AI akan tetap memakai pipeline dan data timnya, tetapi tidak boleh melenceng dari guardrail ini.</p></div></div>{isLoading ? <p className="mt-4 text-[11px] text-[#84918b]">Memuat standar tim…</p> : <div className="mt-4 grid gap-3 sm:grid-cols-2">{fields.map((field) => <label key={field.key} className="settings-field"><span className="settings-field-label">{field.label}</span><span className="text-[10px] leading-4 text-[#93a09a]">{field.hint}</span><textarea value={form[field.key]} onChange={(event) => updateField(field.key, event.target.value)} placeholder={field.placeholder} maxLength={6000} className="soft-input mt-1 min-h-24 w-full resize-y" /></label>)}</div>}<div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-[10px] text-[#899690]">Terakhir disimpan akan berlaku pada pesan berikutnya.</p><button type="button" disabled={saveMutation.isPending || isLoading || Object.values(form).some((value) => value.trim().length < 10)} onClick={() => saveMutation.mutate({ channelId: selectedChannel, ...form })} className="primary-button">{saveMutation.isPending ? "Menyimpan…" : "Simpan standar tim"}</button></div></section>;
}
...
function SettingsView({ onBack, userName, businessName, onBusinessNameChange, persona, onPersonaChange, automations, automationRuns, pipelineTemplates, pipelineTargets }: { onBack: () => void; userName: string; businessName: string; onBusinessNameChange: (name: string) => void; persona: string; onPersonaChange: (persona: string) => void; automations: AutomationRecord[]; automationRuns: AutomationRunRecord[]; pipelineTemplates: PipelineTemplate[]; pipelineTargets: { channelId: string; title: string }[] }) {
  ...
  return <div className="settings-view"><div className="settings-topbar"><button onClick={onBack} className="back-button"><ArrowLeft className="h-4 w-4" /> Kembali ke chat</button><div className="flex items-center gap-2 text-[12px] text-[#9aa39f]"><Settings className="h-4 w-4" /> Workspace</div></div><div className="settings-scroll"><div className="settings-intro"><div><p className="eyebrow">Preferensi ruang kerja</p><h1 className="mt-2 font-display text-[30px] font-bold tracking-[-0.06em] text-[#293834]">Pengaturan</h1><p className="mt-2 max-w-xl text-[13px] leading-6 text-[#74817c]">Atur cara SAKU AI menemani pekerjaanmu. Perubahan di sini hanya berlaku untuk akun dan workspace ini.</p></div><div className="settings-profile"><Avatar initials={userName.slice(0, 1).toUpperCase()} avatarClass="bg-[#d7eee5] text-[#287c68]" size="md" /><div><p className="text-[12px] font-bold text-[#34433e]">{userName}</p><p className="text-[11px] text-[#8b9692]">Pemilik workspace</p></div></div></div><div className="settings-grid"><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e1f2ec] text-[#2a876f]"><Users className="h-4 w-4" /></span><div><h2>Workspace</h2><p>Identitas yang dipakai tim SAKU AI</p></div></div><label className="settings-field block w-full"><span>Nama bisnis</span><div className="soft-input-row"><BriefcaseBusiness className="h-4 w-4 text-[#8ba39a]" /><input aria-label="Nama bisnis" value={businessName} onChange={(event) => onBusinessNameChange(event.target.value)} placeholder="Bisnismu" className="min-w-0 flex-1 bg-transparent text-[12px] font-semibold text-[#40564d] outline-none placeholder:text-[#9da7a3]" /><PenLine className="ml-auto h-3.5 w-3.5 text-[#9da7a3]" /></div></label><label className="settings-field block w-full"><span>Persona AI</span><textarea aria-label="Persona AI" value={persona} onChange={(event) => onPersonaChange(event.target.value)} placeholder="Contoh: hangat, ringkas, dan proaktif" maxLength={1000} className="mt-2 min-h-20 w-full resize-y rounded-xl border border-[#dfece5] bg-white px-3 py-2 text-[12px] leading-5 text-[#40564d] outline-none" /></label></section><TeamStandardsPanel pipelineTargets={pipelineTargets} /><PasswordSettingsCard /><ConnectedAccountsPanel /><MootaIntegrationCard /><GoogleSheetsIntegrationCard /><SmartMatchingRulesPanel /><MemberManagementPanel /><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#fff0db] text-[#bd762b]"><Bell className="h-4 w-4" /></span><div><h2>Notifikasi</h2><p>Atur kapan tim memberi kabar</p></div></div><div className="toggle-row"><div><p>Notifikasi workspace</p><span>Pesan dan update penting</span></div><Toggle checked={notifications} onChange={setNotifications} label="Notifikasi workspace" /></div><div className="toggle-row"><div><p>Suara pesan</p><span>Putar suara saat ada pesan baru</span></div><Toggle checked={sounds} onChange={setSounds} label="Suara pesan" /></div><div className="toggle-row"><div><p>Tampilan ringkas</p><span>Kurangi jarak antar pesan</span></div><Toggle checked={compact} onChange={setCompact} label="Tampilan ringkas" /></div></section><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e9e5f8] text-[#755fa4]"><ShieldCheck className="h-4 w-4" /></span><div><h2>Privasi & data</h2><p>Kontrol data bisnis dan akses tim</p></div></div><button type="button" onClick={exportWorkspace} className="settings-link-row"><span><strong>Ekspor data</strong><small>Unduh salinan informasi workspace</small></span><Download className="h-4 w-4" /></button></section><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e6f0fb] text-[#4f78a5]"><ClipboardList className="h-4 w-4" /></span><div><h2>Template pipeline bisnis</h2><p>Alur siap pakai untuk pekerjaan inti</p></div></div><div className="space-y-3">{pipelineTemplates.map((template) => <div key={template.id} data-testid={`pipeline-template-${template.id}`} className="rounded-xl border border-[#e1eee8] bg-[#fbfefc] p-3"><p className="text-[12px] font-bold text-[#34433e]">{template.department} · {template.name}</p><p className="mt-1 text-[11px] leading-5 text-[#7e8b86]">{template.description}</p><div className="mt-3 flex items-center gap-2"><select aria-label={`Pilih divisi untuk ${template.name}`} value={targetChannel} onChange={(event) => setTargetChannel(event.target.value)} className="min-w-0 flex-1 rounded-lg border border-[#dfece5] bg-white px-2.5 py-2 text-[10px] font-semibold text-[#5b7067]">{pipelineTargets.map((target) => <option key={target.channelId} value={target.channelId}>{target.title}</option>)}</select><button type="button" data-testid={`apply-template-${template.id}`} disabled={applyTemplateMutation.isPending || !pipelineTargets.length} onClick={() => applyTemplateMutation.mutate({ templateId: template.id, channelId: targetChannel, pipelineName: (pipelineNames[template.id] ?? template.name).trim() })} className="rounded-lg bg-[#2d9478] px-3 py-2 text-[10px] font-bold text-white disabled:opacity-50">Pakai</button></div></div>)}</div>{appliedMessage && <p role="status" className="mt-3 rounded-xl bg-[#f0faf5] px-3 py-2 text-[11px] font-semibold text-[#2b836b]">{appliedMessage}</p>}</section><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#dff4eb] text-[#2c8a70]"><Zap className="h-4 w-4" /></span><div><h2>Automasi aktif</h2><p>Perintah yang sudah disimpan</p></div></div>{automations.length ? automations.map((automation) => <div key={automation.id} className="rounded-xl border border-[#e1eee8] bg-[#fbfefc] p-3"><p className="text-[12px] font-bold text-[#34433e]">{automation.name}</p><p className="mt-1 text-[11px] text-[#7e8b86]">{automation.description}</p></div>) : <div className="rounded-xl bg-[#f6faf8] p-3 text-[11px] text-[#7e8b86]">Belum ada automasi tersimpan.</div>}</section><section className="settings-card"><div className="settings-card-heading"><span className="settings-card-icon bg-[#e8eef8] text-[#5e75a7]"><Activity className="h-4 w-4" /></span><div><h2>Riwayat eksekusi</h2><p>Jejak kerja automasi yang sudah berjalan</p></div></div>{automationRuns.length ? automationRuns.slice(0, 5).map((run) => <div key={run.id} className="rounded-xl border border-[#e1eee8] bg-[#fbfefc] p-3"><div className="flex items-center justify-between"><p className="text-[12px] font-bold text-[#34433e]">Run #{run.id}</p><span className="rounded-full bg-[#e3f4ec] px-2 py-1 text-[10px] font-bold text-[#2b836b]">{run.status}</span></div><p className="mt-1 text-[11px] leading-5 text-[#7e8b86]">{run.output}</p></div>) : <div className="rounded-xl bg-[#f6faf8] p-3 text-[11px] text-[#7e8b86]">Belum ada eksekusi tersimpan.</div>}</section><HelpCenterPanel /><section className="settings-card"><button type="button" onClick={() => void logout().then(() => toast.success("Kamu sudah keluar dari workspace."))} className="logout-row"><LogOut className="h-4 w-4" /> Keluar dari workspace</button></section></div></div></div>;
}
